...
public class HappyPathTests
{
    ...
    private readonly TransactionContextProvider<TestStep, SingleStepTestContext> _contextProvider;
    ...
    public HappyPathTests()
    {
        ...
        var stateRestorerMock = new Mock<IStateRestorer<TestStep, SingleStepTestContext>>();
        _contextProvider = new TransactionContextProvider<TestStep, SingleStepTestContext>();
    ...
    {
        // Arrange
        var request = new TestRequest<SingleStepTestContext> { TransactionId = "trx-happy-1" };
        var response = new TestResponse { Success = true };

        _mementoStoreMock.Setup(x => x.GetLatestAsync(request.TransactionId, default))
            .ReturnsAsync((Memento<TestStep, SingleStepTestContext>?)null);
        ...
        _mementoStoreMock.Verify(x => x.SaveAsync(
            ...
            It.IsAny<SingleStepTestContext>(), // Verify that *any* state object was saved.
        ...
        _contextProvider.Context.Value.Should().Be(123);
    }
}
