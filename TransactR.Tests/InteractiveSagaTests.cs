...
namespace TransactR.Tests
{
    ...
    {
        private readonly Mock<IMementoStore<TestState, TestStep>> _mementoStoreMock;
        ...
        {
            _mementoStoreMock = new Mock<IMementoStore<TestState, TestStep>>();
            _stateRestorerMock = new Mock<IStateRestorer<TestState>>();
            _contextProvider = new TransactionContextProvider<TestSagaContext>();
            var loggerMock = new Mock<ILogger<TransactionalBehaviorBase<TestRequest, TestResponse, TestSagaContext, TestState, TestStep>>>();
        ...
        public async Task ExecuteAsync_WhenStartingSaga_ShouldSaveInitialStateAndPreserveMemento()
        {
            ...
            _mementoStoreMock.Setup(x => x.GetLatestAsync(request.TransactionId, default))
                .ReturnsAsync((Memento<TestState, TestStep>?)null);
            ...
            // Assert
            _mementoStoreMock.Verify(x => x.SaveAsync(request.TransactionId, TestStep.StepOne, It.IsAny<TestState>(), default), Times.Once);
            ...
            _contextProvider.Context.Step.Should().Be(TestStep.StepOne);
        }
        ...
        public async Task ExecuteAsync_WhenContinuingSaga_ShouldLoadLatestStateAndComplete()
        {
            ...
            var response = new TestResponse { Success = true };
            var existingState = new TestState { Value = 1 };
            var latestMemento = new Memento<TestState, TestStep>(TestStep.StepOne, existingState);
            ...
            await _sut.Execute(request, (context) =>
            {
                ...
                context.Step.Should().Be(TestStep.StepOne);
            ...
            _mementoStoreMock.Verify(x => x.SaveAsync(request.TransactionId, TestStep.StepOne, existingState, default), Times.Once);
            ...
            _mementoStoreMock.Verify(x => x.SaveAsync(request.TransactionId, TestStep.StepTwo, It.IsAny<TestState>(), default), Times.Never);

            // Verify the context reflects the final state
            _contextProvider.Context.Step.Should().Be(TestStep.StepTwo);
